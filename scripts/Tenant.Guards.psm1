...
function Assert-AzdTenantContext {
    ...
    if ($LASTEXITCODE -ne 0 -or -not $activeTenant) { throw 'Unable to resolve the active Azure CLI tenant.' }
    Assert-TenantMatch -ExpectedTenantId $env:AZURE_TENANT_ID -SubscriptionTenantId $subscriptionTenant.Trim() -ActiveTenantId $activeTenant.Trim()
}
